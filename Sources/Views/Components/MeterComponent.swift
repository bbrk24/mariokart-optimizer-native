    ...
    struct Meter: View {
        ...
        var body: some View {
            VStack(alignment: .leading, spacing: 2) {
                ...
                VStack(spacing: 0) {
                    ...
                    HStack(spacing: 0) {
                        Group {
                            ...
                            fillColor.frame(
                                width: Int(
                                    min(width, 1.0) * Float(idealWidth - 7 * strokeWidth) / 6.0
                                )
                            )

                            Color.clear.frame(
                                width: Int(
                                    max(1.0 - width, 0.0) * Float(idealWidth - 7 * strokeWidth)
                                        / 6.0
                                )
                            )
                            ...
                            fillColor.frame(
                                width: Int(
                                    max(min(width - 1.0, 1.0), 0.0)
                                        * Float(idealWidth - 7 * strokeWidth) / 6.0
                                )
                            )

                            Color.clear.frame(
                                width: Int(
                                    min(max(2.0 - width, 0.0), 1.0)
                                        * Float(idealWidth - 7 * strokeWidth) / 6.0
                                )
                            )
                        ...
                        Group {
                            ...
                            fillColor.frame(
                                width: Int(
                                    max(width - 5.0, 0.0) * Float(idealWidth - 7 * strokeWidth)
                                        / 6.0
                                )
                            )
                            ...
                            Color.clear.frame(
                                width: Int(
                                    min(6.0 - width, 1.0) * Float(idealWidth - 7 * strokeWidth)
                                        / 6.0
                                )
                            )
                ...
                .frame(minHeight: minHeight, idealHeight: maxHeight, maxHeight: Double(maxHeight))
            }
        }
    }
#else
        ...
        func path(in bounds: Path.Rect) -> Path {
            ...
            for i in 1..<6 {
                path =
                    ...
                    .addLine(
                        to: SIMD2(
                            x: offset + width * Double(i) / 6.0 + (bounds.height - strokeWidth)
                                * slant,
                            y: bounds.y + strokeWidth / 2.0
                        )
                    )
            ...
            MeterShape()
                .path(
                    in: Path.Rect(
                        ...
                        width: (bounds.width - 7.0 * strokeWidth) * Double(width / 6.0)
                            + strokeWidth * Double(2.375 + width / 2.0),
                        height: bounds.height - strokeWidth
                    )
                )
        }
